...
{
    public class BookBankController : ControllerBase
    {
        private readonly BookBankService _bankService;

        public BookBankController(IConfiguration configuration)
        {
            var connectionString = configuration.GetConnectionString("DatabaseConfig");
            _bankService = new BookBankService(connectionString);
        }


        [HttpPost("BookBank/Create")]
        public async Task<IActionResult> CreateUser([FromBody] BookBank? bank)
        {
            try
            {
                if (bank == null)
        ...
        [HttpPut("BookBank/Update")]
        public async Task<IActionResult> UpdateUser(int id, [FromBody] BookBank? bank)
        {
            try
            {
                if (id != bank.id)
        ...
        [HttpGet("BookBank/GetById")]
        public async Task<IActionResult> GetUserById(int id)
        {
            var user = await _bankService.GetByIdAsync(id);

            if (user == null)
                return NotFound($"BookBank with ID {id} not found.");
